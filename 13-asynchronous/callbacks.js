...
// It inserts into the document a new, dynamically created, tag <script src="…"> with the given src. The
...
// Let’s say we need to use the new script as soon as it loads. It declares new functions, and we want to
// run them.
//
...
// Naturally, the browser probably didn’t have time to load the script. As of now, the loadScript function
// doesn’t provide a way to track the load completion. The script loads and eventually runs, that’s all.
...
// The onload event is described in the article Resource loading: onload and onerror, it basically executes
// a function after the script is loaded and executed.
//
...
// That’s called a “callback-based” style of asynchronous programming. A function that does something
...
// The convention is:
// 1. The first argument of the callback is reserved for an error if it occurs. Then callback(err)
//    is called.
